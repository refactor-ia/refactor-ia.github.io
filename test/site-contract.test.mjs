...
test('renders the Spanish public landing contract', () => {
  ...
  assert.match($('body').text(), /Modernize with engineering judgment\. No hype\./);
  ...
  assert.match($('meta[name="description"]').attr('content') ?? '', /comunidad/i);
  assert.equal($('link[rel="canonical"]').attr('href'), 'https://refactoria.dev/');
  assert.equal($('meta[property="og:url"]').attr('content'), 'https://refactoria.dev/');
  assert.equal(
  ...
  for (const category of [
    ...
    assert.ok($('main').text().includes(category), `includes the ${category} category`);
  }
  assert.ok(
  ...
  for (const forbiddenTerm of ['holawindev', 'innit', 'equipo', 'team']) {
    assert.ok(
...
test('publishes structured identity data connecting RefactorIA, Juan Barbat and official channels', () => {
  ...
  for (const sameAs of [links.githubOrg, links.youtubeMain, links.youtubeDevs]) {
    assert.ok(org.sameAs.includes(sameAs), `Organization sameAs includes ${sameAs}`);
  }
});
...
test('publishes static fallback, crawl and visual metadata assets', () => {
  ...
  assert.match(robots, /Sitemap: https:\/\/refactoria\.dev\/sitemap-index\.xml/);
