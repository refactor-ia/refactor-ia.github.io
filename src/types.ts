...
export interface SeoProps {
  /** Page title. Rendered as `{title} | {SITE_NAME}`; omit for `SITE_NAME` alone. A title that already contains `SITE_NAME` passes through untouched. */
  title?: string;
  /** Meta description, also used for `og:description` and `twitter:description`. Defaults to `SITE_DESCRIPTION`. */
  description?: string;
  /** Social share image. Defaults to `/og-image.png`. Relative paths resolve against `site` in `astro.config.mjs`. */
  image?: string;
   ...
   */
  noindex?: boolean;
  /** Structured data injected as an inline JSON-LD script, for identity graphs. */
